    ...
    <section className="section-full relative flex items-center overflow-hidden">
      ...
      <div ref={contentRef} className="relative z-10 w-full px-8 lg:px-16 py-16" style={{ marginLeft: '260px' }}>
        <div className="max-w-5xl">
          <span className="animate-item block text-sm text-[#8B8680] mb-2">03</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4">
            {printSizes.map((size) => (
              ...
              >
                ...
                <div className="text-sm font-semibold text-[#1E1E1E] mb-1">{size.name}</div>
                <div className="text-xs text-[#8B8680] mb-2">{size.width}x{size.height} {size.unit}</div>
                <div className="text-sm font-bold text-[#D4A056]">${size.price.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
