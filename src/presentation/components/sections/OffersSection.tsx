...
export default function OffersSection({ section, isActive }: Props) {
  ...
  const offers = (section.content.offers || []).filter((o: OfferItem) => o.active);

  const handleOfferCTA = (offer: OfferItem) => {
    const message = offer.whatsappMessage || `Hola, me interesa la oferta: ${offer.title}`;
    ...
    <section className="section-full relative flex items-center overflow-hidden">
      ...
      <div ref={contentRef} className="relative z-10 w-full px-8 lg:px-16 py-16" style={{ marginLeft: '260px' }}>
        <div className="max-w-4xl">
          <span className="animate-item block text-sm text-[#8B8680] mb-2">06</span>
            ...
            {offers.map((offer: OfferItem) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                  <div>
                    {offer.badge && (
                      <span className={`inline-block text-[11px] font-bold px-3 py-1 rounded-full mb-3 ${badgeColorMap[offer.badgeColor] || badgeColorMap.golden}`}>
                        {offer.badge}
                      </span>
                    )}
                    <h3 className="text-xl font-semibold text-[#1E1E1E]">{offer.title}</h3>
                  </div>
                  <div className="text-right">
                    <div className="text-sm text-[#8B8680] line-through">${offer.originalPrice.toLocaleString()}</div>
                    <div className="text-2xl font-bold text-[#D4A056]">${offer.promotionalPrice.toLocaleString()}</div>
                  </div>
                </div>

                <p className="text-sm text-[#8B8680] mb-4 leading-relaxed">{offer.description}</p>

                {offer.conditions && (
                  <p className="text-xs text-[#8B8680] mb-4 italic">*{offer.conditions}</p>
                )}
