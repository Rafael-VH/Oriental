...
const menuItems = [
  ...
  { id: 'section-2', label: 'Sección 2 - Servicios', icon: Camera, sectionId: 2 },
  { id: 'section-3', label: 'Sección 3 - Impresión', icon: Printer, sectionId: 3 },
  { id: 'section-4', label: 'Sección 4 - Trámites', icon: FileText, sectionId: 4 },
  { id: 'section-5', label: 'Sección 5 - Portafolio', icon: Image, sectionId: 5 },
  { id: 'section-6', label: 'Sección 6 - Ofertas', icon: Tag, sectionId: 6 },
  { id: 'section-7', label: 'Sección 7 - Ubicación', icon: MapPin, sectionId: 7 },
];
...
export default function DashboardSidebar({ activeTab, onTabChange }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Logo */}
      <div className={`pt-6 pb-4 px-4 border-b border-white/10 ${sidebarCollapsed ? 'text-center' : ''}`}>
        {sidebarCollapsed ? (
          <div className="text-[#D4A056] font-display text-lg font-bold">EO</div>
        ) : (
          <div>
            <div className="text-white font-display text-lg font-bold leading-tight">CMS Estudio</div>
      ...
      {/* Bottom buttons */}
      <div className={`p-3 border-t border-white/10 space-y-2 ${sidebarCollapsed ? 'px-1' : ''}`}>
        <a
          href="/"
          target="_blank"
