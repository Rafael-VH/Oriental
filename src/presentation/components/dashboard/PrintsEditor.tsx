...
export default function PrintsEditor({ section }: Props) {
  ...
  const handleUpdate = (id: string, updates: Partial<PrintSizeItem>) => {
    const updated = printSizes.map((p) => (p.id === id ? { ...p, ...updates } : p));
    ...
    <div className="space-y-6">
      ...
      <div className="dashboard-card">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-dashboard text-base font-semibold text-[#2D3748]">Tamaños de Impresión ({printSizes.length})</h2>
          <button
            onClick={handleAdd}
        ...
        <div className="space-y-3">
          {printSizes.map((item) => (
            <div key={item.id} className="border border-[#E8E4E0] rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <div>
                    <label className="text-xs text-[#718096] mb-1 block">Ancho</label>
                    <input
                      type="number"
                      className="dashboard-input"
                      value={item.width}
                      onChange={(e) => handleUpdate(item.id, { width: Number(e.target.value) })}
                    />
                  </div>
                  <div>
                    <label className="text-xs text-[#718096] mb-1 block">Alto</label>
                    <input
                      type="number"
                      className="dashboard-input"
                      value={item.height}
                      onChange={(e) => handleUpdate(item.id, { height: Number(e.target.value) })}
                    />
                  </div>
                  <div>
                    <label className="text-xs text-[#718096] mb-1 block">Unidad</label>
                    <select
                      className="dashboard-input"
                      value={item.unit}
                      onChange={(e) => handleUpdate(item.id, { unit: e.target.value as 'cm' | 'in' })}
                    >
                  ...
                  <div>
                    <label className="text-xs text-[#718096] mb-1 block">Precio ($)</label>
                    <input
                      type="number"
                      className="dashboard-input"
                      value={item.price}
