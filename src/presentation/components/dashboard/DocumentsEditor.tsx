    ...
    <div className="space-y-6">
      ...
      <div className="dashboard-card">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-dashboard text-base font-semibold text-[#2D3748]">Tipos de Documentos ({documents.length})</h2>
          <button
            onClick={handleAdd}
        ...
        <div className="space-y-3">
          {documents.map((doc) => (
            <div key={doc.id} className="border border-[#E8E4E0] rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-xs text-[#718096] mb-1 block">Tiempo de proceso</label>
                    <input
                      type="text"
                      className="dashboard-input"
                      value={doc.processingTime}
                      onChange={(e) => handleUpdate(doc.id, { processingTime: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="text-xs text-[#718096] mb-1 block">Requisitos</label>
                    <textarea
                      className="dashboard-textarea w-full"
                      rows={2}
                      value={doc.requirements}
                      onChange={(e) => handleUpdate(doc.id, { requirements: e.target.value })}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="dashboard-input"
                      value={doc.price}
                      onChange={(e) => handleUpdate(doc.id, { price: Number(e.target.value) })}
                    />
                  </div>
                </div>
                <button
